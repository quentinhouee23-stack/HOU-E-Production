...
export function MiniPlayer() {
  ...
  // 🟢 NOUVEL ÉTAT : Permet de cacher le lecteur pendant un swipe
  ...
  useEffect(() => {
    ...
    const hide = () => setIsHiddenBySwipe(true);
    const show = () => setIsHiddenBySwipe(false);

    window.addEventListener("musicTimeUpdate", handleTimeUpdate);
    window.addEventListener("hideMiniPlayer", hide);
    window.addEventListener("showMiniPlayer", show);

    return () => {
      window.removeEventListener("musicTimeUpdate", handleTimeUpdate);
      window.removeEventListener("hideMiniPlayer", hide);
      window.removeEventListener("showMiniPlayer", show);
    };
  }, []);
